...
class Unet3DModelInception():
	...
	def inception_layer(self, X, filters):
		X1 = Conv3D(filters=filters[0], kernel_size=1, strides=(1,1,1), padding='same', activation='relu')(X)

		X33 = Conv3D(filters=filters[1][0], kernel_size=1, strides=(1,1,1), padding='same', activation='relu')(X)
		X33 = Conv3D(filters=filters[1][1], kernel_size=3, strides=(1,1,1), padding='same', activation='relu')(X33)

		X55 = Conv3D(filters=filters[2][0], kernel_size=1, strides=(1,1,1), padding='same', activation='relu')(X)
		X55 = Conv3D(filters=filters[2][1], kernel_size=5, strides=(1,1,1), padding='same', activation='relu')(X55)

		X_Max = MaxPooling3D(pool_size=(3,3,3),strides=(1,1,1), padding="same")(X)
		X_Max = Conv3D(filters=filters[3], kernel_size=1, strides=(1,1,1), padding='same', activation='relu')(X_Max)
	...
	def build_model(self, input_shape):
	    ...
	    X = Conv3D(filters=16, kernel_size=3, strides=(1,1,1), padding='same', activation='relu')(X_input)
	    X = Conv3D(filters=32, kernel_size=3, strides=(1,1,1), padding='same', activation='relu')(X_input)
	    ...
	    U3 = Conv3D(256, 2, padding='same', activation='relu')(UpSampling3D(size = (2,2,2), dim_ordering="tf")(D4))
	    U3 = Concatenate()( [D3, U3])
	    U3 = self.inception_layer(U3, filters=[32, [32, 64], [8, 16], 32])


	    U2 = Conv3D(256, 2, padding='same', activation='relu')(UpSampling3D(size = (2,2,2), dim_ordering="tf")(U3))
	    U2 = Concatenate()( [D2, U2])
	    U2 = self.inception_layer(U2, filters=[32, [32, 64], [8, 16], 32])

	    U1 = Conv3D(128, 2, padding='same', activation='relu')(UpSampling3D(size = (2,2,2), dim_ordering="tf")(U2))
