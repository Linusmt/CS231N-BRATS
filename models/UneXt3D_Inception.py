...
class Unet3DModelInception():
	...
	def inception_layer(self, X, filters):
		...
		X1_ = Conv3D(filters=filters[0], kernel_size=1, strides=(1,1,1), padding='same', activation='tanh')(X)

		X33_ = Conv3D(filters=filters[1][0], kernel_size=1, strides=(1,1,1), padding='same', activation='tanh')(X)
		X33_ = Conv3D(filters=filters[1][1], kernel_size=3, strides=(1,1,1), padding='same', activation='tanh')(X33_)

		X55_ = Conv3D(filters=filters[2][0], kernel_size=1, strides=(1,1,1), padding='same', activation='tanh')(X)
		X55_ = Conv3D(filters=filters[2][1], kernel_size=5, strides=(1,1,1), padding='same', activation='tanh')(X55_)

		X_Max_ = MaxPooling3D(pool_size=(3,3,3),strides=(1,1,1), padding="same")(X)
		X_Max_ = Conv3D(filters=filters[3], kernel_size=1, strides=(1,1,1), padding='same', activation='tanh')(X_Max_)
